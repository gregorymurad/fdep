...
def fetch_latest_data_iot(x):
    if x.count_documents({}) == 0:  # checking if the collection has any documents
        return pd.DataFrame()  # return an empty DataFrame if there are no documents in the collection
    else:
        records = x.find().sort("timestamp", -1).limit(1000)  # fetch the latest 1000 records
        return pd.DataFrame(list(records))  # convert the records to DataFrame and return

def get_df():
    df = fetch_latest_data_iot(st.session_state['collection'])
    # If the DataFrame is empty, return it early
    # print("DFFFFF here: ", df, type(df))
    # print("*****")
    if df.empty:
...
if 'collection' in st.session_state:
    ...
    if message == "Success :)":
        if menu == "Map - Real Time Visualization":
            ...
            def plotFinally():
                st_autorefresh(interval=10000, key='plot refresh')
                df,message = get_df()
                my_map = create_map(df)  # Create the map
                folium_static(my_map,width=1550, height=400)
            ...
            # tab1, tab2 = st.tabs(["Updating Live 📊","Interactive Map 🗺️"])
            with st.container():
        ...
        if menu == "Charts - Real Time Visualization":
            def plotRefresh():
                st_autorefresh(interval=5000, key='plot refresh')
        ...
        with st.expander("See table with Raw Data", expanded=False):
            def show_dataframe():
                st_autorefresh(interval=5000, key='df_refresh')
                st.dataframe(df)
            show_dataframe()
        ...
        st.warning("No data available yet for this page⏳")
else:
